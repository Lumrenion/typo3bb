...
class PostFactory {
    ...
    public static function processAttachments(Post $post, $attachments) {
        ...
        $settings = $configurationManager->getConfiguration(ConfigurationManagerInterface::CONFIGURATION_TYPE_FRAMEWORK, 'typo3bb')['settings'];
     ...
     */
    public static function _processAttachmentsInternal(Post $post, $attachments, $attachmentsPath, string $duplicationBehaviour = DuplicationBehavior::RENAME, bool $removeOriginal = true) {
        $objectManager = self::_getObjectManagerInstance();
        ...
        foreach ($attachments as $attachment) {
            if (is_array($attachment)) {
                if ($attachment['error'] == UPLOAD_ERR_OK) {
                    $movedFile = self::addFileToFolder($attachmentsFolder, $attachment['tmp_name'], hash_file('md5', $attachment['tmp_name']), $duplicationBehaviour, $removeOriginal);
     ...
     */
    public static function addFileToFolder(Folder $folder, string $filePath, string $newName = '', string $duplicationBehavior = DuplicationBehavior::RENAME, bool $removeOriginal = true) {
        if ($removeOriginal) {
